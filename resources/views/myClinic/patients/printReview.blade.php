...
<div class=" h-100 p-1 " style="direction: rtl;text-align:right ;font-family:'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif">
    ...
    <div class=" pt-2 pb-4" style="display:flex">
        <a href="{{route('Clinic.index')}}"  class="text-center" style="width:30%;">
            <div>
                <p class="text-xs text-gray-700 p-0 m-0">عيادة الدكتور {{$patientReview->patient->user->name}}</p>
                <p class="text-xs text-gray-700 p-0 m-0">
                    @if ($patientReview->patient->user->doctor_info->university)
                        جامعة {{$patientReview->patient->user->doctor_info->university}}
                    @endif
                    @if ($patientReview->patient->user->doctor_info->med_specialty)
                        - {{$patientReview->patient->user->doctor_info->med_specialty}}
                    @endif
                </p>
                <p class="text-xs text-gray-700 p-0 m-0" style="direction:ltr">{{$patientReview->patient->user->mobile}}
                    @if ($patientReview->patient->user->doctor_info->address)
                        / {{$patientReview->patient->user->doctor_info->address}}
                    @endif
                </p>
            </div>
        </a>
        <div  class="text-center" style="width:50%;">
            ...
            <a href="{{route('Clinic.patientProfile',$patientReview->patient->patient_slug)}}">
                <p class="h5 text-gray-900 p-0 m-0">اسم المريض : <b>{{$patientReview->patient->patient_name}}</b></p>
            </a>
        </div>
        <div style="text-align: center;width: 20%;">
            ...
            <p class="text-xs text-gray-700 p-0 m-0">تاريخ الإدخال : {{$patientReview->patient->created_at->format('D d-m-Y')}}</p>
            <p class=" text-xs text-gray-900  p-0 m-0"></p>
    ...
    <div class="table-responsive"  style="border-radius: 10px; border: 1px solid;" >
        <table class="table table-bordered mb-0">
            ...
            <tbody>
                <tr>
                    @if ($patientReview->patient->gender)
                        <td class=" text-gray-900">@if ($patientReview->patient->gender == 'male') {{'ذكر'}} @elseif ($patientReview->patient->gender == 'female') {{'أنثى'}} @endif</td>
                    @else
                    ...
                    @if ($patientReview->patient->age && $patientReview->patient->age!= date('Y') )
                        <td class=" text-gray-900" style="direction:rtl">{{date('Y') - $patientReview->patient->age .' سنة'}}</td>
                    @else
                        <td class=" text-gray-900" style="direction:rtl;line-height: 46px;">---------------------</td>
                    @endif
                    @if ($patientReview->patient->blood_type)
                        <td class=" text-gray-900" style="direction:rtl">{{$patientReview->patient->blood_type}}</td>
                    @else
                        <td class=" text-gray-900" style="direction:rtl;line-height: 46px;">---------------------</td>
                    @endif
                    @if ($patientReview->patient->smoking)
                        <td class=" text-gray-900" style="direction:rtl">@if ($patientReview->patient->smoking == 'negative') {{'سلبي'}} @elseif ($patientReview->patient->smoking == 'positive')  {{'إيجابي'}} @endif</td>
                    @else
                        <td class=" text-gray-900" style="direction:rtl;line-height: 46px;">---------------------</td>
                    @endif
                    @if ($patientReview->patient->relationship)
                        <td class=" text-gray-900" style="direction:rtl">@if ($patientReview->patient->relationship == 'married') {{'متزوج'}} @elseif ($patientReview->patient->relationship == 'single')  {{'أعزب'}} @endif</td>
                    @else
                        <td class=" text-gray-900" style="direction:rtl;line-height: 46px;">---------------------</td>
                    @endif
                    @if ($patientReview->patient->child_count)
                        <td class=" text-gray-900" style="direction:rtl">{{$patientReview->patient->child_count}}</td>
                    @else
                        <td class=" text-gray-900" style="direction:rtl;line-height: 46px;">---------------------</td>
                    @endif
                </tr>
    ...
    <div class="pt-2">
        <div class="w-100 d-inline-flex px-4">
            <div class="w-100 text-center d-inline-flex" style="width: 30%;">
                ...
                @if ($patientReview->patient->phone)
                    <p class="  text-gray-900  p-0 m-0" style=" direction:ltr">{{$patientReview->patient->phone}}</p>
                @else
            ...
            <div class="w-100 text-center d-inline-flex" style="width: 20%;">
                ...
                @if ($patientReview->patient->patient_job)
                    <p class="  text-gray-900  p-0 m-0" style=" direction:ltr">{{$patientReview->patient->patient_job}}</p>
                @else
                    <p class="  text-gray-900  p-0 m-0" style=" direction:ltr;line-height: 46px;"> ---------------------------------</p>
                @endif
            </div>
            <div class="w-100 text-center d-inline-flex" style="width: 50%;">
                ...
                @if ($patientReview->patient->patient_address)
                    <p class="  text-gray-900  p-0 m-0" style=" direction:ltr;">{{$patientReview->patient->patient_address}}</p>
                @else
                    <p class="  text-gray-900  p-0 m-0" style=" direction:ltr;line-height: 46px;"> ----------------------------------------</p>
                @endif
            </div>
            ...
            <tbody>
                <tr style="height: 150px" >
                    <td class="border-right-dark">
                        {{$patientReview->patient->older_surgery}}
                    </td>
                    <td  class="border-right-dark">
                        {{$patientReview->patient->older_sicky}}
                    </td>
                    <td  class="border-right-dark">
                        {{$patientReview->patient->older_sensitive}}
                    </td>
                    <td  class="border-right-dark">
                        {{$patientReview->patient->permanent_medic}}
                    </td>
                    <td  class="border-right-dark">
                        {{$patientReview->patient->patient_state}}
                    </td>
                </tr>
            </tbody>
        </table>
